...
scatter_labels = None
  .type = str
  .help = DiffBragg steps to plot scatter for, for example "0-2" for 0, 1 & 2. 
  .help = Use -1 for to plot scatter before diffBragg.
"""
...
def set_default_return(default_path: str) -> Callable:
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs):
            ...
            return os.path.expandvars(default_path) if res is None else res
        return wrapper
    return decorator
...
def expand_integer_ranges(ranges_str: str) -> List[int]:
    """Convert string of int ranges (e.g. "1-3,5") into list ([1, 2, 3, 5])"""
    ranges = []
    for range_str in ranges_str.split(','):
        if '-' in range_str:
            start, _, end = range_str.partition('-')
            ranges.extend(list(range(int(start), int(end)+1)))
        else:
