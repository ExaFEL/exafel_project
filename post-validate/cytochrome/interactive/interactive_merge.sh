...
# scaling model
export PDB_PATH=`ls $MODULES/exafel_project/kpp-frontier/$SAMPLE/*.pdb`
if [ "$PDB_PATH" == "" ]; then
  export PDB_PATH=`ls $MODULES/exafel_project/kpp-sim/$SAMPLE/*.pdb`
fi
# covariance pickle
export COV_PATH=`ls $MODULES/exafel_project/kpp-frontier/$SAMPLE/*.pickle`
if [ "$COV_PATH" == "" ]; then
  export COV_PATH=`ls $MODULES/exafel_project/kpp-sim/$SAMPLE/*.pickle`
fi
...
filter.unit_cell.cluster.covariance.file=$COV_PATH
