...
N_FIRST_PASS=$3 # number of shots to process in order to gauge restraints
...
export SG=`iotbx.crystal_symmetry_from_any $PDB_PATH | grep space_group | sed "s/.*=//; s/).*//"`
if [ "$SAMPLE" == "cry11ba" ]; then
  SIGMAS="1 1 1"
else
  SIGMAS="1 1"
fi
