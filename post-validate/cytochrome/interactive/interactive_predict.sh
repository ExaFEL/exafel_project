...
iotbx.crystal_symmetry_from_any $PDB_PATH > sym.out
export UCELL=`grep unit_cell sym.out | sed "s/.*(//; s/).*//; s/ //g"`
export SG=`grep space_group sym.out | sed "s/.*='//; s/').*//"`
