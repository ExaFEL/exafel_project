...
N=$((2*n_thousand))  # NERSC number of nodes: 1 node/1000 shots (Q=regular); 2 nodes/1000 shots (Q=debug)
...
if requested 1; then sbatch -J$job $GPU -t 30  -o${odir}/sim.out -e${odir}/sim.err ${script_dir}/general_sim.sh $length $nshot $detdist $pdb $odir ; fi
...
# optional ens.hopper:
#if requested 8; then sbatch -J$job $GPU -t 30 -o${odir}/ens.out -e${odir}/ens.err ${script_dir}/general_ensHopper.sh $dmin $odir ; fi
