...
JOB_ID_SIM=$(chained_sbatch $SCRIPTS/thermolysin_500k_sim.sh None)
...
# arguments passed to the thermolysin_500k_*.sh script.
JOB_ID_INDEX=$(chained_sbatch $SCRIPTS/thermolysin_500k_index.sh $JOB_ID_SIM $JOB_ID_SIM)

# Merge stills processing results, pending completion of stills_process
JOB_ID_MERGE=$(chained_sbatch $SCRIPTS/thermolysin_500k_merge.sh $JOB_ID_INDEX $JOB_ID_INDEX)

# Split results of stills processing and prepare file list for diffBragg.
JOB_ID_SPLIT=$(chained_sbatch $SCRIPTS/thermolysin_500k_split.sh $JOB_ID_INDEX $JOB_ID_INDEX)
...
JOB_ID_HOPPER=$(chained_sbatch $SCRIPTS/thermolysin_500k_stage1.sh "${JOB_ID_SPLIT}:${JOB_ID_MERGE}" $JOB_ID_SPLIT $JOB_ID_MERGE)

# Complete spot prediction and interation of predictions with newly refined models.
JOB_ID_PREDICT=$(chained_sbatch $SCRIPTS/thermolysin_500k_predict.sh $JOB_ID_HOPPER $JOB_ID_HOPPER)

# Run diffBragg stage 2 (global refinement of structure
# factors against pixel data) on integration results.
JOB_ID_STAGE2=$(chained_sbatch $SCRIPTS/thermolysin_500k_stage2.sh $JOB_ID_PREDICT $JOB_ID_INDEX $JOB_ID_MERGE $JOB_ID_PREDICT)
