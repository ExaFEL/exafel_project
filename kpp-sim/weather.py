...
phil_scope = parse('''
  main_log_dir = .
    .type = path
    .help = directory of the main slurm log
  rank_log_dir = .
    .type = str
    .help = path to find the per-rank logs and output data
  jobid = None
    .type = str
    .help = root name of the job, same as $SLURM_JOB_ID on Perlmutter
  num_nodes = 1
    .type = int
...
def get_MPI_time():
  with open(os.path.join(params.rank_log_dir,params.jobid,"rank_0.log"),"r") as F:
...
def get_py_time():
  with open(os.path.join(params.rank_log_dir,params.jobid,"rank_0.log"),"r") as F:
    ...
    return time.mktime(time.strptime(ctime,"%Y-%m-%d %H:%M:%S.%f")), elapsed
def get_log(params):
  log_file = os.path.join(params.main_log_dir,"%s.out"%(params.jobid))
...
def run(params):
  ...
  counter = 0
  datum = None
  root=os.path.join(params.rank_log_dir,params.jobid)
