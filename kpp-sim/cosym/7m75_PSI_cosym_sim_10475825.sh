...
crystal {
  structure=pdb
  pdb.code=7m75 # Keable PS I to 2.75 Angstrom
  length_um=28. # increase crystal path length
}
detector {
  tiles=multipanel
  reference=$MODULES/exafel_project/kpp-sim/t000_rg002_chunk000_reintegrated_000000.expt
  offset_mm=110 # increase distance (offset from reference) to set lower resolution
}
output {
  format=h5
}
